...
var Populate = module.exports = function Populate(connection, docs, joins) {
  ...
  // Hold values that are only used internally and can be removed
  this.internalValues = [];
...
Populate.prototype.buildDocuments = function buildDocuments(cb) {
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    self._generatePopulatedFields();

    // Run async iteration in the correct context
    function joinDocs(item) {
      return self._joinRelatedDocuments.call(self, item);
    }

    Promise.all(self.docs)
    .then(joinDocs)
    .then(function () {
      resolve(utils.rewriteIds(self.docs));
    })
    .error(function(err) {
      reject(err);
    });
...
Populate.prototype._normalizeJoins = function normalizeJoins(joins) {
  ...
  for(var i = 0; i < joins.length; i++) {
    ...
    if(!join.select) {
      this.internalValues.push(join.childKey);
      continue;
    }
...
Populate.prototype._findRelatedDocuments = function findRelatedDocuments(joinSet, cb) {
  ...
  populateCollection.find(where).toArray(function(err, childDocs) {
    ...
    // Cleanse any internal only values
    self._cleanValues(childDocs);
...
/**
 * Clean Documents of their internal values
 *
 * Removes values that are specific to mongo and only used internally. These are currently only
 * the embedded realated record arrays for many to many associations.
 *
 * @param {Array|Object} documents
 * @api private
 */

Populate.prototype._cleanValues = function cleanValues(documents) {
  var self = this;
  ...
  documents.forEach(function(doc) {
    self.internalValues.forEach(function(val) {
      if(!hasOwnProperty(doc, val)) return;
      delete doc[val];
    });
  });
...
Populate.prototype._joinRelatedDocuments = function joinRelatedDocuments(document, cb) {
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    self.populatedFields.forEach(function(field) {
