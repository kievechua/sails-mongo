...
var Collection = module.exports = function Collection(definition) {
  ...
  this.joinTable = definition.meta ? definition.meta.junctionTable : false;
...
Collection.prototype.destroy = function destroy(criteria, cb) {
  ...
  connection.open(function() {
    ...
    if(!self.joinTable) {
      return connection.database.collection(self.identity)
      .remove(query.criteria.where, function(err, results) {
        connection.database.close(function() {
          if(err) return cb(err);
